Require Import List Omega Numbers.BinNums QArith Psatz.
...
Proof.
  cbv. intros r H. decompose sum H; subst; ppo.
Qed.
...
Proof. cbv; auto. intros _. auto. discriminate. Qed.
...
Definition qif (f : function) (args : list Qpos) := match f with
  | main => qmax (qmax (5*q(qsuml args)) (8+q(qsuml args)) +q 1) 2 +q 1 (* max(max(5X,X+8) + 1, 2) + 1 *)
            (* args here has only one element, X, so suml args = X *)
  | mult => qsuml args +q (hd (Qpos_of_nat 0) args)  (* 2X+Y *)
...
- apply Qle_trans with (qsuml l).
  ...
  + simpl. qpos_tac. lra.
- apply Qle_trans with (qmaxl l).
  + apply qmaxl_is_qmax;trivial.
  + qpos_tac. lra.
- apply Qle_trans with (qmaxl l).
  + apply qmaxl_is_qmax;trivial.
  + qpos_tac. lra.
- apply Qle_trans with (qsuml l).
  + apply in_le_qsuml;trivial.
  + qpos_tac. lra.
- apply in_le_qsuml in Hin. qpos_tac; try lra.
- apply in_le_qsuml; trivial.
Qed.

Ltac qif_monotonic_tac :=
  repeat (apply forall2_le_qsuml || apply forall2_le_qmaxl ||
          apply qmax_le_compat || apply Qpos_plus_le_compat || apply Qpos_mult_le_compat ||
...
Proof.
  intros f xs ys Hfor.
  destruct f; unfold qif; qif_monotonic_tac; try lra.
  - destruct xs; destruct ys; inversion Hfor; auto. lra.
  - unfold Qposle. lra.
Qed.
...
Proof.
  ...
  unfold cs; destruct QIy; cbn; repeat ring_simplify; qpos_tac; try lra.
Qed.
