(** * Utilities library *)
Require Import Bool QArith Arith Max Omega Psatz List NPeano Permutation.
...
(*
Notation "x ≤ y" := (Qle x y) (at level 70, y at next level).
Notation "x ≤ y ≤ z" := (x ≤ y ∧ y ≤ z)%Q (at level 70, y at next level).
Notation "x < y ≤ z" := (x < y ∧ y ≤ z)%Q (at level 70, y at next level).
Notation "x ≤ y < z" := (x ≤ y ∧ y < z)%Q (at level 70, y at next level). *)
...
Open Scope Q.
Definition Qpos := {q : Q | 0 <= q}%Q.

Program Definition Qpos_plus : Qpos -> Qpos -> Qpos := Qplus.
Next Obligation.
intros [q H] [q' H'].
simpl.
eapply Qle_trans with (y := 0 + 0); [|apply Qplus_le_compat];
auto with *.
Defined.

Program Definition Qpos_mult : Qpos -> Qpos -> Qpos := Qmult.
Next Obligation.
intros [q H] [q' H'].
simpl.
eapply Qle_trans with (y := 0 + 0); [|apply Qmult_le_0_compat];
auto with *.
Defined.

Definition Qpos_to_Q (q : Qpos) : Q := proj1_sig q.
Coercion Qpos_to_Q :  Qpos>->Q.

Program Definition Qpos_of_nat (n : nat) : Qpos := inject_Z (Z.of_nat n).
Next Obligation.
intro n; destruct n; simpl; unfold inject_Z; auto with *.
Qed.

Lemma Qpos_of_nat_plus_compat  (a b : nat):
  Qpos_of_nat (a+b) == Qpos_plus (Qpos_of_nat a) (Qpos_of_nat b).
Proof.
unfold Qpos_of_nat.
simpl.
unfold inject_Z.
unfold Qplus, Qmult, Qeq.
simpl.
ring_simplify.
apply Nat2Z.inj_add.
Qed.

Program Fixpoint qsuml (l : list Qpos) : Qpos :=
match l as l return Qpos with
| [] => 0
| n :: l' => Qpos_plus n (qsuml l')
end.
Next Obligation. intros _. simpl. auto with *. Defined.

Lemma qsuml_map_le A (f g : A -> Qpos) l :
  (forall x : A, In x l -> f x <= g x) ->
 qsuml (map f l) <= qsuml (map g l).
Proof.
induction l.
- auto with *.
- intros. apply Qplus_le_compat; auto with *.
Qed.

Lemma in_le_qsuml : ∀ (n : Qpos) (l : list Qpos), n ∈ l → n <= qsuml l.
Proof.
intros n l.
...
- destruct Hin as [Heq | Hin].
  + subst; simpl.
    eapply Qle_trans with (y := n + 0); [ ring_simplify; auto with *|].
    apply Qplus_le_compat; auto with *.
    destruct qsuml; trivial.
  + simpl.
    eapply Qle_trans with (y := 0 + n); [ ring_simplify; auto with *|].
    apply Qplus_le_compat; auto with *.
    destruct a; simpl; trivial.
Qed.

Definition qmax (a b : Qpos) := if Qlt_le_dec a b then b else a.

Lemma qmax_le_l (n m : Qpos) : n <= qmax n m.
Proof. unfold qmax; case Qlt_le_dec; intro H; auto with *. Qed.

Lemma qmax_le_r (n m : Qpos) : m <= qmax n m.
Proof. unfold qmax; case Qlt_le_dec; intro H; auto with *. Qed.

Program Fixpoint qmaxl (l : list Qpos) : Qpos :=
  match l return Qpos with
  | nil => 0
  | n :: l' => let (s, _) := qmaxl l' in qmax n s
  end.
Next Obligation. intros _. simpl. auto with *. Defined.
Next Obligation.
intros l [a Ha] l' n Hn. trivial.
Defined.

Lemma qmaxl_is_qmax : ∀ (l : list Qpos) (n : Qpos), n ∈ l → n <= qmaxl l.
Proof.
induction l; intros n H.
- inversion H.
- destruct H as [Ha | Hl].
  + subst a. simpl. destruct qmaxl. apply qmax_le_l.
  + simpl; destruct qmaxl.
    eapply Qle_trans; [ | apply qmax_le_r].
    now apply IHl.
Qed.

Definition Qposle : Qpos -> Qpos -> Prop := Qle.

Lemma qsuml_suml l : Qpos_of_nat (suml l) == qsuml(map Qpos_of_nat l).
Proof.
induction l; unfold Qpos_of_nat.
- auto with *.
- simpl.
...
Lemma qsuml_map_const: ∀ (A : Type) (n : Qpos) (l : list A), qsuml (map (λ _ : A, n) l) == (Qpos_mult n (Qpos_of_nat (length l))).
Proof.
induction l; unfold Qpos_mult, Qpos_of_nat; simpl.
- ring.
- rewrite IHl, Zpos_P_of_succ_nat.
  unfold Qpos_mult, Qpos_of_nat, Z.succ; simpl.
  rewrite inject_Z_plus.
  ring.
Qed.

Lemma Qpos_pos (q : Qpos) : Qle 0 q.
Proof. now destruct q. Qed.

Lemma Qpos_of_nat_le_compat a b : (a <= b)%nat -> Qpos_of_nat a <= Qpos_of_nat b.
Proof.
intro Hle.
unfold Qpos_of_nat, Qle; simpl.
unfold inject_Z.
omega.
Qed.

Lemma qmax_lub
     : forall (n m : Qpos) (p : Q), (n <= p) -> (m <= p) -> (qmax n m <= p).
Proof.
intros n m p Hn Hm.
unfold qmax.
case Qlt_le_dec; intro H.
- now subst.
- trivial.
Qed.

Lemma Qpos_of_nat_max n m: Qpos_of_nat (Nat.max n m) = qmax (Qpos_of_nat n) (Qpos_of_nat m).
Proof.
case (le_lt_dec n m); intro Hltle.
- rewrite Nat.max_r by trivial. unfold qmax. case Qlt_le_dec; intro Hltle'; trivial.
  unfold Qpos_of_nat in *. simpl in *.
  rewrite <- Zle_Qle, <- Nat2Z.inj_le in Hltle'.
  replace n with m; trivial. auto with *.
- rewrite Nat.max_l by auto with *. unfold qmax. case Qlt_le_dec; intro Hltle'; trivial.
  unfold Qpos_of_nat in *. simpl in *.
  rewrite <- Zlt_Qlt, <- Nat2Z.inj_lt in Hltle'.
  replace n with m; trivial. auto with *.
Qed.

Lemma proj1_qmax q r: proj1_sig (qmax q r) = if Qlt_le_dec q r then r else q.
Proof. unfold qmax. now case (Qlt_le_dec q r). Qed.

Lemma qmax_lt_iff : ∀ (n m : Qpos) (p : Q), p < qmax n m ↔ p < n ∨ p < m.
Proof.
intros n m p; split; unfold qmax; case Qlt_le_dec; intros Hc H; auto with *.
...
Lemma Qle_plus_r: ∀ (n p : Q) (m : Q), 0 <= m -> p <= n -> p <= n + m.
Proof.
intros n p m H H'.
eapply Qle_trans with (y := p + 0).
- ring_simplify; auto with *.
- apply Qplus_le_compat; auto with *.
Qed.

Lemma forall2_le_qsuml :
  forall (l1 l2 : list Qpos), Forall2 Qposle l1 l2 -> (qsuml l1 <= qsuml l2)%Q.
Proof.
intros.
induction H;auto;simpl; [auto with * | apply Qplus_le_compat; trivial].
Qed.

Lemma qmax_le_compat : ∀ n m p q : Qpos, Qposle n m → Qposle p q → Qposle (qmax n p) (qmax m q).
Proof.
intros n m p q Hnm Hpq.
unfold qmax. unfold Qposle in *.
case Qlt_le_dec; case Qlt_le_dec; lra.
Qed.

Lemma forall2_le_qmaxl :
  forall (l1 l2 : list Qpos), Forall2 Qposle l1 l2 -> qmaxl l1 <= qmaxl l2.
Proof.
intros.
induction H.
- simpl; auto with *.
- simpl. destruct (qmaxl l) as [q1 Hq1]; destruct (qmaxl l') as [q2 Hq2].
  simpl in *. apply qmax_le_compat;trivial.
Qed.

Lemma Qpos_plus_le_compat: forall x y z t : Qpos, (x <= y)%Q -> (z <= t)%Q -> (Qpos_plus x z <= Qpos_plus y t)%Q.
Proof. intros (x&Hx) (y&Hy) (z&Hz) (t&Ht). simpl. apply Qplus_le_compat. Qed.

Lemma Qpos_mult_le_compat: forall x y z t : Qpos, (x <= y)%Q -> (z <= t)%Q -> (Qpos_mult x z <= Qpos_mult y t)%Q.
Proof.
intros (x&Hx) (y&Hy) (z&Hz) (t&Ht). simpl. intros H1 H2.
apply Qle_trans with (x * t).
- apply Qle_lteq in Hx. destruct Hx as [Hx | Hx].
  + now apply Qmult_le_l.
  + rewrite <- Hx. lra.
- apply Qle_lteq in Ht. destruct Ht as [Ht | Ht].
  + now apply Qmult_le_r.
  + rewrite <- Ht. lra.
Qed.

Lemma Qmax_le_compat: forall x y z t : Qpos, (x <= y)%Q -> (z <= t)%Q -> (qmax x z <= qmax y t)%Q.
Proof. intros (x&Hx) (y&Hy) (z&Hz) (t&Ht). unfold qmax; simpl. do 2 case Qlt_le_dec; intros; simpl; lra. Qed.

End Forall2.

Infix "*q" := Qpos_mult (at level 40).
Infix "+q" := Qpos_plus (at level 40).

(* destructs positive rationals as rational and proof *)
Ltac qpos_tac :=
  unfold qmax; cbn;
  repeat match goal with
         | |- context [if ?a then ?b else ?c ] => case a; cbn
         | |- context [proj1_sig ?a] =>
           let b := fresh "b" in let Hb := fresh "Hb" in let Hbpos := fresh "Hbpos" in
             case_eq a; intros b Hbpos Hb; simpl in *; try ring_simplify in Hb;
             try (rewrite Hb in *; cbn in *)
         | |- ?a -> ?b => lra || (let H := fresh "H" in intro H; try ring_simplify in H)
         end;
  try ring_simplify.

Coercion Qpos_of_nat : nat>->Qpos.
